...
class DeviceManager(QObject):
    ...
    def _probe_camera_status(self, camera_cfg: Optional[Dict]) -> str:
        ...
        probe_source, backend = prepare_camera_source(camera_cfg, width, height, fps)
        ...
        try:
            ...
            if backend_flag is None:
                ...
                cap = cv2.VideoCapture(probe_source, backend_flag)
            ...
            return "online" if ret and frame is not None else "offline"
        except Exception:
            # Fall back to filesystem check for path-based sources
            if isinstance(probe_source, str) and Path(probe_source).exists():
                return "online"
            return "offline"
    ...
    def _match_cameras_to_config(self, cameras: List[Dict]) -> Dict[str, str]:
        ...
        front_cfg_full = self.config.get("cameras", {}).get("front", {})
        wrist_cfg_full = self.config.get("cameras", {}).get("wrist", {})
        front_config = front_cfg_full.get("index_or_path", "")
        wrist_config = wrist_cfg_full.get("index_or_path", "")
        ...
        if not front_found:
            if front_config:
                status = self._probe_camera_status(front_cfg_full)
                self.camera_front_status = status
                self.camera_status_changed.emit("front", status)
                if status == "online":
                    assignments["front"] = front_config
            else:
                self.camera_front_status = "empty"
                self.camera_status_changed.emit("front", "empty")
        ...
        if not wrist_found:
            if wrist_config:
                status = self._probe_camera_status(wrist_cfg_full)
                self.camera_wrist_status = status
                self.camera_status_changed.emit("wrist", status)
                if status == "online":
                    assignments["wrist"] = wrist_config
            else:
                self.camera_wrist_status = "empty"
                self.camera_status_changed.emit("wrist", "empty")
