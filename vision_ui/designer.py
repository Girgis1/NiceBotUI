...
class CameraStream:
    ...
    def _pause_hub_for_preview(self) -> None:
        if not CameraStreamHub or self._hub_pause_active:
            return

        try:
            self._hub_pause_ctx = CameraStreamHub.paused()
            if self._hub_pause_ctx is not None:
                token = self._hub_pause_ctx.__enter__()
                if token:
                    self._hub_pause_active = True
                else:
                    # No active hub; clean up immediately.
