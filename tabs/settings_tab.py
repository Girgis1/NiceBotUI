...
class SettingsTab(QWidget):
    ...
    def go_home(self):
        ...
        if self._home_thread and self._home_thread.isRunning():
            self.status_label.setText("⏳ Home move already running...")
    ...
    def _on_home_finished(self, success: bool, message: str) -> None:
        ...
        if success:
            final_message = message or f"✓ Moved to home position at velocity {velocity}"
        else:
            detail = message or "Unknown error"
            final_message = detail if detail.startswith("❌") else f"❌ Error: {detail}"
